...
pub async fn start_external_watcher_etag(
    ...
    info!(
        "Starting ETag-based external resource watcher for: {}",
        external_endpoint
    );
    ...
    task::spawn(async move {
        loop {
            ...
            match poll_external_resource_etag(
                &http_client,
            ...
            .await
            {
                ...
                Some(PollResult::Changed {
                    ...
                    debug!(
                        "New data received (first 5 objects): {:?}",
                        &listing_data.0.iter().take(5).collect::<Vec<_>>()
                    );
...
enum PollResult {
    Changed {
        new_etag: String,
        listing_data: ObjectListing,
    },
    NotModified,
}

/// Fetches the resource conditionally using the ETag.
async fn poll_external_resource_etag(
    client: &Client,
    external_endpoint: String,
    etag: Option<&str>,
) -> Option<PollResult> {
    ...
    match response.status() {
        ...
        StatusCode::OK => {
            ...
            let new_etag = match response.headers().get("ETag") {
                ...
                None => {
                    error!(
                        "Successful response (200) but no ETag header found. Cannot track state."
                    );
                    return None;
                }
            };
        ...
        StatusCode::NOT_MODIFIED => Some(PollResult::NotModified),
